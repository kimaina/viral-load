...
numerical_var =c(
  ...
  "prop_defaulted_apptmts","prop_bad_adherence", "num_encounters","vl_count_1_log"
...
#```{r  cache=F}
logit.rl.cv <-train(virologic_failure2 ~.,data=train_DF, method = "glmnet", 
                    trControl = train_control,metric = "Sens",
                    tuneGrid = expand.grid(alpha = .4,
                                           lambda = .04))


md2.glmnet=generateClassificationStats(train_DF, logit.rl.cv, cutoff=.5, 
                                       modelName="Logit", predictName=predictName)
